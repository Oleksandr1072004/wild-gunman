...
let sfxIntro = new Audio('assets/sfx/intro.m4a');
let sfxWait = new Audio('assets/sfx/wait.m4a');
let sfxFire = new Audio('assets/sfx/fire.m4a');
let sfxShot = new Audio('assets/sfx/shot.m4a');
let sfxWin = new Audio('assets/sfx/win.m4a');
let sfxDeath = new Audio('assets/sfx/death.m4a');
...
// Додано ці змінні на початок файлу
const aimElement = document.getElementById('aim');
let isShooting = false;

// Додано цю функцію для оновлення позиції прицілу
function updateAimPosition(e) {
  if (!isShooting) {
    aimElement.style.left = `${e.clientX}px`;
    aimElement.style.top = `${e.clientY}px`;
  }
}

// Додано цю функцію для обробки пострілу
function handleShootEffect() {
  isShooting = true;
  aimElement.classList.add('shooting');

  setTimeout(() => {
    aimElement.classList.remove('shooting');
    isShooting = false;
  }, 300);
}

// Додано ці обробники подій в кінці файлу, після ініціалізації гри
document.addEventListener('mousemove', updateAimPosition);
document.addEventListener('click', handleShootEffect);

// Також модифіковано функцію handlePlayerShot, щоб вона викликала ефект пострілу
function handlePlayerShot(event) {
  if (!state.isDuelActive || !state.isGameActive) return;

  const gunmanRect = elements.gunman.getBoundingClientRect();
  const clickedOnGunman =
    event.clientX >= gunmanRect.left &&
    event.clientX <= gunmanRect.right &&
    event.clientY >= gunmanRect.top &&
    event.clientY <= gunmanRect.bottom;

  // Додано цей рядок для ефекту пострілу
  handleShootEffect();
